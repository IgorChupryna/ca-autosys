...
<div class="container">
    ...
    <div w3-include-html=${agent.fullDesc}></div>
    <script>
        w3.includeHTML();
    </script>
</div>
...
<div class="container">
    ...
    <video width="480" height="320" controls>
        ...
        <source src=${agent.pathVideo} type="video/mp4">
    </video>
</div>
...
<div class="container">
    ...
    <div class="row">
        <div class='list-group gallery'>
            <c:forEach items="${agent.imgs}" var="list">

            <c:if test="${fn:length(agent.imgs) eq 1}"><div class='col-sm-12'></c:if>
                <c:if test="${fn:length(agent.imgs) eq 2}"><div class='col-sm-6'></c:if>
                    <c:if test="${fn:length(agent.imgs) eq 3}"><div class='col-sm-4'></c:if>
                        <c:if test="${fn:length(agent.imgs) gt 3}"><div class='col-sm-3 col-xs-6 col-md-3 col-lg-3'></c:if>
        ...
        <div class="chatContainer">
            ...
            <div class="chatHistoryContainer">
                <ul class="formComments">
                    <c:forEach items="${agent.comments}" var="list" varStatus="myIndex">
                                            ...
                                            <c:if test="${login == list.user}">
                                                <div id="commentactions-${myIndex.index}" class="comment-actions">
                                                    <form id="del" method="POST"
                                                          action="/delComment?name=<c:out value="${agent.name}"></c:out>&id=<c:out value="${list.id}"></c:out>&table=Agent">
            ...
            <div class="input-group input-group-sm chatMessageControls">
                ...
                <form id="send" method="POST"
                      action="/addComment?name=<c:out value="${agent.name}"></c:out>&id=<c:out value="${agent.id}"></c:out>&table=Agent">
