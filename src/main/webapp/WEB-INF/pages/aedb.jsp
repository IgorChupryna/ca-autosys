...
<div class="container">
    ...
    <div w3-include-html=${aedb.fullDesc}></div>
    <script>
        w3.includeHTML();
    </script>
</div>
...
<div class="container">
    ...
    <video width="480" height="320" controls>
        ...
        <source src=${aedb.pathVideo} type="video/mp4">
    </video>
</div>
...
<div class="container">
    ...
    <div class="row">
        <div class='list-group gallery'>
            <c:forEach items="${aedb.imgs}" var="list">

            <c:if test="${fn:length(aedb.imgs) eq 1}"><div class='col-sm-12'></c:if>
                <c:if test="${fn:length(aedb.imgs) eq 2}"><div class='col-sm-6'></c:if>
                    <c:if test="${fn:length(aedb.imgs) eq 3}"><div class='col-sm-4'></c:if>
                        <c:if test="${fn:length(aedb.imgs) gt 3}"><div class='col-sm-3 col-xs-6 col-md-3 col-lg-3'></c:if>
        ...
        <div class="chatContainer">
            ...
            <div class="chatHistoryContainer">
                <ul class="formComments">
                    <c:forEach items="${aedb.comments}" var="list" varStatus="myIndex">
                                            ...
                                            <c:if test="${login == list.user}">
                                                <div id="commentactions-${myIndex.index}" class="comment-actions">
                                                    <form id="del" method="POST"
                                                          action="/delComment?name=<c:out value="${aedb.name}"></c:out>&id=<c:out value="${list.id}"></c:out>&table=AEDB">
            ...
            <div class="input-group input-group-sm chatMessageControls">
                ...
                <form id="send" method="POST"
                      action="/addComment?name=<c:out value="${aedb.name}"></c:out>&id=<c:out value="${aedb.id}"></c:out>&table=AEDB">
